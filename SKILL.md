...
description: 个人行程与报销 V1.0.18 | 配置化版本，支持分享与在线升级
version: 1.0.18
...
1. **文件自动识别** (`invoice_auto_organizer.py`)
   ...
   - 三层去重机制：① 发票号去重 ② 行程单与发票交叉去重 ③ 无发票号时按日期+类别+金额通用去重
   - 自动关联行程、生成台账
...
2. **行程自动整理** (`trip_auto_organizer.py`)
   ...
   - 生成发票清单、行程总览、报销单模板
   - **行程去重**：按日期范围检测已有行程，已存在则复用并刷新发票（清空旧文件重新复制），不再创建重复目录
...
3. **邮件下载发票** (`download_invoices.py` + `email_manager.py`，可选）
   ...
   - **双层去重**：① 邮件 Message-ID 去重（已下载过的邮件自动跳过）② 附件内容 MD5 去重（与 01 待分类 + 03 已完成比对，已有相同文件不重复保存）
...
5. **新增行程** (`import_trips.py`)
   - 触发命令：`新增行程`
   - **WorkBuddy AI 对话场景（主要）**：AI 在对话中收集行程数据，通过 stdin 管道传入脚本执行
   - **终端直接运行场景**：直接运行脚本，弹出文本输入窗口，粘贴 Tab 分隔行程数据
   - **文件批量导入场景**：`python3 import_trips.py --file trips.txt`
...
- `2026-06-15_机票比价图_50.00_广州-上海_张三_0624_WB_076.png`（机票比价截图，金额=机场燃油，购买方=乘机人）

> 机票比价图：识别关键词"直飞"，从"直飞"后提取航班日期作为文件名日期，提取"机场燃油"金额，提取"乘机人"姓名代替购买方，提取城市-城市格式路线。属于非报销凭证（参考类）。
