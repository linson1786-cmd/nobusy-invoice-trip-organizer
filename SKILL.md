...
description: 个人行程与报销（配置化版本，支持分享）
trigger: ["跑一次发票整理", "有新行程", "从邮件下载发票", "发票整理", "行程整理", "上传", "初始化", "初始化设置", "重置", "下载发票", "收发票"]
---
...
| `upload_files.py` | 文件选择上传脚本（入口含版本检查） |
