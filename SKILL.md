...
1. **文件自动识别** (`invoice_auto_organizer.py`)
   - 触发命令：`文件识别`
   - **预分类**：先判断文件性质 → 正式发票 vs 普通文件 → 走不同校验标准（V1.0.49）
...
分类前先判断文件性质，不同性质走不同校验标准：

```
文件进入 → is_invoice_file(text, ext)
  ├─ True  → 正式发票 → 严格校验
  └─ False → 普通文件 → 宽松校验
```

**正式发票判定标准**（含 `STRICT_INVOICE_MARKERS` 任一关键词即判定）：

| 分类 | 关键词 |
|------|--------|
| 发票编号 | 发票号码、发票代码、机器编号 |
| 日期/金额 | 开票日期、价税合计 |
| 税务/票种 | 税号、电子发票、增值税、铁路电子客票、电子客票 |
| 交易双方 | 销售方、购买方、收款人、复核人、开票人 |
| 英文 | ` Invoice`、`Receipt`、`Tax` |

### 类别清单

| 分类 | 文件性质 | 典型文件 | 关键词示例 |
|------|---------|---------|-----------|
| **机票比价图** | 普通文件 | 携程/飞猪航班预订截图 | 直飞、乘机人、单程、值机柜台、登机口 |
| **高铁比价图** | 普通文件 | 12306/携程火车票截图 | 乘车人、车次、二等座、12306、检票口 |
| **住宿比价图** | 普通文件 | 携程/去哪儿酒店截图 | 房型、大床房、双床房、连住、每晚 |
| **结账单** | 普通文件 | 酒店前台打印账单 | 结账单、水单 |
| **行程单** | 普通文件 | 滴滴/高速行程单 | 行程单、行程报销单、出行记录 |
| **住宿** | 正式发票 | 酒店电子发票 | 住宿、酒店、华住、全季、房费 |
| **礼品** | 正式发票 | 购物电子发票 | 礼品、日用品、丝巾、纺织、礼盒 |
| **高速费** | 正式发票 | ETC/通行费发票 | 高速、通行费、车辆通行、ETC |
| **机票** | 正式发票 | 航空公司电子发票 | 机票、航空、登机牌、退票费、保险 |
| **机票(保险)** | 正式发票 | 平安/众安保险发票 | 机票类 + 保险/航意险 |
| **充电费** | 正式发票 | 充电桩发票 | 充电费、蔚来、换电、充电桩 |
| **油电类** | 正式发票 | 加油/国网发票 | 加油、汽油、柴油、中石化、燃油费 |
| **滴滴打车** | 正式发票 | 滴滴/代驾电子发票 | 滴滴、didi、代驾、交通运输服务 |
| **高铁** | 正式发票 | 铁路电子客票 | 火车票、高铁、铁路、电子客票 |
| **餐饮** | 正式发票 | 餐饮/酒类电子发票 | 餐饮、餐费、饭店、酒、红酒、白酒 |

### 校验规则

| 文件性质 | 要求 |
|---------|------|
| **正式发票** | 日期 + 金额 + 发票特征词（三要素缺一不可） |
| **普通文件（比价图）** | 专用提取器处理日期/金额，提取失败 → 02 待核实 |
| **普通文件（其他）** | 日期 + 金额 + 类别匹配即可，不要求发票特征词 |
| **无法分类** | 所有类别均不匹配 → 02 待核实 |

### 子类型判定

| 基础类别 | 触发条件 | 最终标签 |
|---------|---------|---------|
| 机票 + 含"保险" | 保险/航意险 | **机票(保险)** |
| 行程单 + 含"滴滴/didi/代驾" | 滴滴/代驾 | **滴滴打车(行程单)** |
| 行程单 + 含"高速/通行费" | 高速费 | **高速费(行程单)** |
| 结账单 + 含"酒店/住宿/房费" | 酒店账单 | **住宿(结账单)** |
