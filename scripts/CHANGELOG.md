...
- 修复 `config_template.py` 缺少 `BASE_ROOT` / `INVOICE_ROOT` / `TRIP_ROOT` 等路径变量的问题
  - 新增 `import os` 和自动计算逻辑，从 `OBSIDIAN_VAULT` + 相对路径反推所有路径
  - 手动复制模板为 config.py 也能正常工作，不再依赖 setup.py 生成
- 修复 `trip_auto_organizer.py` 和 `invoice_auto_organizer.py` 中硬编码 `NoBusy-Demo` 测试路径的问题
  - 所有 fallback 路径改为从 `OBSIDIAN_VAULT` + `INVOICE_BASE_REL` / `TRIP_BASE_REL` 反推
  - 报销单模板路径从 config 的 `REIMBURSEMENT_TEMPLATE` 读取，不再硬编码
- 修复全局"个人报销单"与"员工报销单"目录名不一致的问题
  - 统一为"员工报销单"，涉及 config_template.py、trip_auto_organizer.py、setup.py、invoice_auto_organizer.py、SKILL.md 共 13+ 处

### 新增

- `import_trips.py` 新增 `--file` / `-f` 参数，支持从文件读取行程数据
  - 三种输入模式：`--file` > stdin 管道 > GUI 窗口，按优先级自动选择
  - 用法：`python3 import_trips.py --file trips.txt`
