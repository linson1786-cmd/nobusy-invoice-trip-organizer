...
# 如果 config.py 未提供路径变量，从 OBSIDIAN_VAULT + 相对路径反推
if 'BASE_ROOT' not in dir():
    _vault = os.path.expanduser(globals().get('OBSIDIAN_VAULT', '~'))
    _invoice_rel = globals().get('INVOICE_BASE_REL', '个人行程与报销/01 发票整理')
    _trip_rel = globals().get('TRIP_BASE_REL', '个人行程与报销/02 行程与员工报销单')
    BASE_ROOT = os.path.join(_vault, _invoice_rel)
    INVOICE_ROOT = BASE_ROOT
    DONE_DIR = os.path.join(BASE_ROOT, "03 已完成")
    TRIP_ROOT = os.path.join(_vault, _trip_rel)
...
# 报销单模板路径（从 config.py 的 REIMBURSEMENT_TEMPLATE 读取，无则留空跳过模板复制）
