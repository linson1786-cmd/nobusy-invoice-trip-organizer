...
- **行程整理：月度餐饮同步** 📋
  - `copy_dining_to_monthly()` 增加清理逻辑：删除月度餐饮目录中 03 已完成 已不存在的文件
  - 少的补（新餐饮发票自动复制）+ 多的删（已删除发票自动清理）
  - 行程文件夹已有 `clear_existing=True` 实现同步
# 版本变更日志
...
- **02 待核实文件名前缀失败原因** 📋
  - 移入 02 待核实的文件自动加 `[原因]` 前缀
  - 一眼可知：`[无日期]` `[无金额]` `[无类别]` `[无航班日期]` `[缺发票特征]` 等
  - 替换原文件名难以判断的问题
...
- **机票比价图 ≥3 关键词扩充** ✈️
  - `_ORDINARY_FLIGHT_MARKERS` 新增长 `"经济舱"`、`"机场"`、`"航班"`
  - 从 5 个 → 8 个，OTA 截图覆盖率更高
...
- **普通文件二级分类：≥3 关键词精确匹配** 📋
  - 普通文件（非正式发票）先过 `_classify_ordinary_file()` 精确检测：
    - 住宿(结账单)：酒店/结账单/房号/入住日期/离店日期/水单/入离日期 ≥3 → 结账单
    - 机票比价图：直飞/单程/机建燃油/托运行李/航空 ≥3 → 机票比价图
  - 不足 3 个时走标准 CATEGORY_RULES
  - 正式发票不受影响（≥3 STRICT_INVOICE_MARKERS 跳过此路径）

### 设计原则

```
文件 → is_invoice_file() ≥3 markers
  ├─ True  → 正式发票 → 标准 CATEGORY_RULES（跳过比价图）
  └─ False → _classify_ordinary_file()
              ├─ ≥3 结账单词 → 结账单 → 住宿(结账单)
              ├─ ≥3 机票比价图词 → 机票比价图
              └─ None → 标准 CATEGORY_RULES
```
...
| 场景 | 命中 | 结果 |
|------|------|------|
| 酒店水单（酒店+水单+入离日期） | 3/7 | 住宿(结账单) ✅ |
| 机票OT截图（直飞+机建燃油+托运） | 3/5 | 机票比价图 ✅ |
| OTA截图仅2词 | 2/5 | 走标准规则 ✅ |
| 真高铁票 | 跳过（正式发票） | 高铁 ✅ |
