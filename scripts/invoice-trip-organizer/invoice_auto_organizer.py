...
_DEFAULT_SUBTYPE_RULES = [
        ...
        ("行程单", ["滴滴", "打车", "网约车", "didi", "代驾", "交通运输服务", "客运服务费"], "(行程单)"),
...
def extract_pdf_text(filepath):
    """V1.0.46: 提取PDF文本并合并CJK换行。
    pdfminer 提取时可能在CJK字符间插入\\n，导致'国内航空'→'国内航\\n空'，影响关键词匹配。
    """
    try:
...
def extract_date_from_text(text):
    # V1.0.53: XML转PDF格式 "开票日期: 2026-06-15 01:28:04"
    m = re.search(r'开票日期[：:]\s*(\d{4})[-/](\d{1,2})[-/](\d{1,2})', text)
    if m:
        y, mo, d = int(m.group(1)), int(m.group(2)), int(m.group(3))
        if 2020 <= y <= 2030 and 1 <= mo <= 12 and 1 <= d <= 31:
            return f"{y}-{mo:02d}-{d:02d}"
    # 通用格式: "开票日期: 2026年06月15日"
...
def extract_date_for_flight_comparison(text):
    ...
    #    先试完整日期 YYYY-MM-DD
    # V1.0.42: 路线分隔符增加"一"（OCR 常将"-"误识为"一"，如"广州一上海"）
    m = re.search(
        r'(\d{4})[年\-/.](\d{1,2})[月\-/.](\d{1,2})'
        r'[\s\S]{0,40}'
        r'(?:[\u4e00-\u9fa5]{2,}[-—→一]+[\u4e00-\u9fa5]{2,}|飞行|小时)',
        text)
    if m:
        y, mo, d = int(m.group(1)), int(m.group(2)), int(m.group(3))
        if 2020 <= y <= 2030 and 1 <= mo <= 12 and 1 <= d <= 31:
    ...
    # V1.0.41: 放宽行首限制 — OCR 可能将"直飞"识别为乱码（如 Els)），
    # 导致日期前面有非数字前缀，原 (?:^|\n\s*) 要求行首太严格
    # V1.0.42: 路线分隔符增加"一"
    m = re.search(
        r'(?:^|\n|[^\d])'
...
def _clean_filename_for_classify(filename):
    """V1.0.46: 精确移除文件名中标准格式位置的类别标签，防止旧名污染分类
    
    标准命名: YYYY-MM-DD_类别_金额[_路线][_购买方][_状态]_序号.ext
    只移除位置1的类别（按_分隔），不做全文替换，避免子串副作用。
    如: '2026-06-21_机票(保险)_40.00_中山康天_WB_234.pdf'
    → 移除位置1的'机票(保险)' → 2026-06-21__40.00_中山康天_WB_234.pdf
    """
    ...
    # 位置0: YYYY-MM-DD, 位置1: 类别
    cat_in_name = segments[1]
    if cat_in_name in CATEGORY_LABELS_IN_FILENAME or cat_in_name in VALID_CATEGORIES:
        segments[1] = ""
...
def reprocess_review_files():
    ...
    for f in files[:]:
        ...
        if STANDARD_NAME_RE.match(fname):
            ...
            # V1.0.53: _rr 后缀文件已被回炉过，不再移回（避免反复 OCR）
            print(f"   ⏭️  {fname[:50]} 已回炉过，跳过")
            files.remove(f)
            skipped += 1
